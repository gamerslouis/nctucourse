...
const Category = ({ catIdx, classes, cname, content, contentKey, controlFlag, id, loading, setAnchor }) => {
  ...
  const credits = loading ? 0 : contentKey.map((itemId, idx) => (filter(content[idx], controlFlag) ? (getAltCredit(itemId) || content[idx].cos_credit) : 0)).reduce((accu, cur) => (accu + cur), 0)
  return (
